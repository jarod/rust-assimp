...
extern {
     ...
     *///}}}
    fn aiImportFile(fname: *const c_char, flags: c_uint) -> *const Scene;
...
mod test {
    ...
    fn test_import_file() {
        unsafe {
            let scene = "cube.dae".with_c_str(|s| import::aiImportFile(s, 0) );
            let err_msg = CString::new(import::aiGetErrorString(), false);
            println!("{}", (*scene).get_root_node().get_name());
            println!("{}", err_msg);
        }
    }
}
